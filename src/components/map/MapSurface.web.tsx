...
/**
 * `react-native-maps` has no web implementation, so the browser gets a
 * schematic rather than a broken tile view: true bearings and true distances
 * plotted around you, with range rings for scale. It answers the same question
 * the map answers — what is around me, and how far — without pretending to be
 * something it is not.
 */
export function MapSurface({ origin, markers, selectedId, onSelect, radiusMetres }: MapSurfaceProps) {
  const { c } = useTheme();
